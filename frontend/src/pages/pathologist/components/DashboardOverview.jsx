        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody>
            {cases.map((c) => {
              ...
              const isUrgent = c.urgency === 'Urgent' || c.urgent;
              const patientLabel = c.patientName || c.patient || c.id;
              const caseType = c.type || c.testType || 'General Pathology';
              const recTime = c.time || c.submittedDate || 'Today';
              ...
              return (
                ...
                >
                  <td className="px-5 py-3 font-semibold text-gray-800">{patientLabel}</td>
                  <td className="px-3 py-3 text-gray-600">{caseType}</td>
                  ...
                  <td className="px-3 py-3 text-gray-500">{recTime}</td>
                  <td className="px-3 py-3 text-gray-500 font-medium">{c.status}</td>
...
function SelectedCaseDetail({ caseData, onSignDB, onSignContext }) {
  ...
  const handleSignReport = () => {
    if (!caseData) return;
    if (onSignDB) {
      onSignDB(caseData.id, remarks || 'Reviewed and digitally signed.', results);
    }
    if (onSignContext) {
      onSignContext(caseData.id);
    }
  };
...
function RecentSignedReports({ signedCases }) {
  const reports = useMemo(() => {
    if (signedCases && signedCases.length > 0) {
      return signedCases.slice(0, 3).map(c => ({
        name: `Signed Report ${c.id}`,
...
export default function DashboardOverview() {
  const { cases, pathologyCases: dbCases, signReport, signPathologyReportDB } = useLims()

  // Merge context cases with MongoDB database cases
  const mergedCases = useMemo(() => {
    const combined = [...(dbCases || []), ...(cases || []).map((c, index) => ({
      id: c.id,
      patientName: c.patientName || `Patient ${c.id}`,
      type: c.type || 'General Pathology Review',
      urgency: c.urgency || 'Urgent',
      urgent: c.urgency === 'Urgent',
      time: c.time || 'Today 11:30 AM',
      status: c.status || 'Technician Submitted',
      findings: ''
    }))];
  ...
  const [selectedCaseId, setSelectedCaseId] = useState(mergedCases[0]?.id || 'PT98765')

  const selectedCase = mergedCases.find((c) => c.id === selectedCaseId) || mergedCases[0] || null
  const urgentCount = mergedCases.filter((c) => c.urgency === 'Urgent' || c.urgent).length
  const signedCases = mergedCases.filter((c) => c.status === 'Signed & Finalized')
      ...
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="col-span-3 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-5">
            <RecentSignedReports signedCases={signedCases} />
        ...
        <div className="col-span-2">
          <SelectedCaseDetail
            ...
            onSignDB={signPathologyReportDB}
            onSignContext={signReport}
          />
        </div>
      </div>
    </div>
